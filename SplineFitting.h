...
namespace Eigen
{
   ...
   * Note: This is a technical interpretation of the referenced article as
   * the algorithm contained therein could not be made to function as written.
   * Note: If the interpolated spline resulting from derivative constraints
   * contains unexpected slack curvature, the underlying issue is likely an
   * insuficcient original dataset.  Enrichen the origianl interpolant dataset
                                    ...
                                    KnotVectorType& knots)
  {
    ...
    // plus 2 knots, one for each of the first and last parameters.
    ...
    // Number of knots (one for each control point and derivative) plus spline order.
    DenseIndex numKnots = numParameters + numDerivatives + degree + 2; 
  ...
  template <typename KnotVectorType, typename ParameterVectorType, typename IndexArray>
  void KnotsWithDerivatives(const ParameterVectorType& parameters,
                            const unsigned int degree,
                            const IndexArray& derivativeIndices,
                            KnotVectorType& knots)
  {
    ...
    DenseIndex order = degree + 1;
    DenseIndex halfOrder = order / 2;

    DenseIndex numConditions = numParameters + numDerivatives;

    DenseIndex numKnots = numConditions + order;
    knots.resize(numKnots);

    Scalar tval1 = parameters(0);
    Scalar tval2 = parameters(numParameters - 1);

    knots.head(order).fill(tval1);
    knots.tail(order).fill(tval2);

    DenseIndex ki = order;

    std::vector<Scalar> epar;

    // Build epar
    for (DenseIndex i = 0; i < parameters.size(); ++i)
    {
      epar.push_back(parameters(i));

      for (DenseIndex j = 0; j < derivativeIndices.size(); ++j)
        if (i == derivativeIndices(j))
        {
          epar.push_back(parameters(i));
          break;
        }
    ...
    DenseIndex startMultiplicity = 1;
    for (; epar[startMultiplicity - 1] == epar[startMultiplicity]; startMultiplicity++)
    {
    }

    DenseIndex endMultiplicity = 1;
    for (DenseIndex i = numConditions - 2; epar[i] == epar[i + 1]; i--)
      endMultiplicity++;

    DenseIndex denseIndex0 = static_cast<DenseIndex>(0);
    Scalar scalar2 = static_cast<Scalar>(2);
    Scalar scalar3 = static_cast<Scalar>(3);

    if (order % 2 == 0)
    {
      // The order is even.
      // Place the internal knots at the parameter values.
      Scalar dummy1 = (epar[startMultiplicity] + tval1) / scalar2;
      Scalar dummy2 = (epar[ -endMultiplicity - 1] + tval2) / scalar2;
      if (dummy1 == dummy2)
      {
        dummy1 = (epar[startMultiplicity] + tval1 + tval1) / scalar3;
        dummy2 = (epar[numConditions - endMultiplicity - 1] + tval2 + tval2) / scalar3;
      }

      for (DenseIndex i = 0; i < startMultiplicity - halfOrder; ++i, ++ki)
        knots(ki) = dummy1;

      for (DenseIndex kpar = startMultiplicity + std::max(denseIndex0, halfOrder - startMultiplicity), 
             kstop = numConditions - endMultiplicity
             - std::max(denseIndex0, halfOrder - endMultiplicity);
           kpar < kstop; ++kpar, ++ki)
        knots(ki) = epar[kpar];

      for (DenseIndex i = 0; i < endMultiplicity - halfOrder; ++i, ++ki)
        knots(ki) = dummy2;
    }
    else
    {
      ...
      if (startMultiplicity - halfOrder > 0)
      {
        Scalar delta = (epar[startMultiplicity] - tval1) / (startMultiplicity - halfOrder + 1);
        Scalar dummy = tval1 + delta;
        for (DenseIndex i = 0; i < startMultiplicity - halfOrder;
             ++i, dummy += delta, ++ki)
          knots(ki) = dummy;
      }
      ...
      for (DenseIndex kpar = startMultiplicity + std::max(denseIndex0, halfOrder - startMultiplicity),
             ...
             - std::max(denseIndex0, halfOrder - endMultiplicity);
           kpar < kstop; ++kpar, ++ki)
        knots(ki) = (double) 0.5 *(epar[kpar] + epar[kpar + 1]);

      if (endMultiplicity - halfOrder > 0)
      {
        Scalar delta = (tval2 - epar[numConditions -endMultiplicity - 1])
          / (endMultiplicity - halfOrder + 1);
        Scalar dummy = epar[numConditions -endMultiplicity - 1] + delta;
        for (DenseIndex i = 0; i < endMultiplicity - halfOrder; ++i, dummy += delta, ++ki)
          knots(ki) = dummy;
      }
    }
  }
  ...
  struct SplineFitting
  {
    ...
    static SplineType Interpolate(
        const PointArrayType& pts, DenseIndex degree, ParameterVectorType& parameters);
     ...
     **/
    template <typename PointArrayType>
    static SplineType InterpolateImpl(
        const PointArrayType& pts, DenseIndex degree, const ParameterVectorType& parameters);
    ...
    static SplineType InterpolateWithDerivatives(const PointArrayType& points,
                                                 ...
                                                 const IndexArray& derivativeIndices,
                                                 const unsigned int degree,
                                                 ParameterVectorType& paprameters,
                                                 const KnotRoutine knotRoutine);
     ...
     */
    template <typename PointArrayType, typename IndexArray>
    static SplineType InterpolateWithDerivativesImpl(const PointArrayType& points,
                                                     const PointArrayType& derivatives,
                                                     const IndexArray& derivativeIndices,
                                                     const unsigned int degree,
  ...
  template <typename SplineType>
  template <typename PointArrayType>
  SplineType SplineFitting<SplineType>::InterpolateImpl(
      const PointArrayType& pts, DenseIndex degree, const ParameterVectorType& parameters)
  {
    ...
    for (DenseIndex i=1; i<n-1; ++i)
    {
      ...
      A.row(i).segment(span-degree, degree+1) = SplineType::BasisFunctions(parameters[i], degree, knots);
    }
    A(0,0) = 1.0;
    A(n-1,n-1) = 1.0;
  ...
  SplineType SplineFitting<SplineType>::Interpolate(
      const PointArrayType& pts, DenseIndex degree, ParameterVectorType &parameters)
  {
    if (pts.cols() == 2 && degree > 1)
    {
      Matrix<typename SplineType::Scalar, SplineType::Dimension, 1> direction
        = pts.col(1) - pts.col(0);

      typename SplineType::Scalar magnitude
        = direction.norm() / static_cast<typename SplineType::Scalar>(degree);
      direction.normalize();

      typename SplineType::ControlPointVectorType controlPoints;
      controlPoints.resize(SplineType::Dimension, degree + 1);

      controlPoints.col(0) = pts.col(0);
      controlPoints.col(controlPoints.cols() - 1) = pts.col(1);

      for (DenseIndex i = 1; i < degree; ++i)
        controlPoints.col(i)
          = pts.col(0).matrix() + static_cast<typename SplineType::Scalar>(i) * magnitude * direction;

      typename SplineType::KnotVectorType knots(2 * (degree + 1));
      
      knots.head(degree + 1).fill(0);
      knots.tail(degree + 1).fill(1);
      
      parameters.resize(2);
      parameters(0) = 0;
      parameters(1) = 1;

      return SplineType(knots, controlPoints);
    }
    else
    {
      ChordLengths(pts, parameters);
      return InterpolateImpl(pts, degree, parameters);
    }
  }
  ...
  template <typename PointArrayType, typename IndexArray>
  SplineType 
  SplineFitting<SplineType>::InterpolateWithDerivativesImpl(const PointArrayType& points,
                                                            const PointArrayType& derivatives,
                                                            const IndexArray& derivativeIndices,
                                                            const unsigned int degree,
                                                            const ParameterVectorType& parameters,
                                                            const KnotRoutine knotRoutine)
  {
    ...
    switch (knotRoutine)
    {
    case averagingRoutine:
      KnotAveragingWithDerivatives(parameters, degree, derivativeIndices, knots);
      break;
    case parametersRoutine:
      KnotsWithDerivatives(parameters, degree, derivativeIndices, knots);
      break;
    }
                                                        ...
                                                        const KnotRoutine knotRoutine)
  {
    ...
    return InterpolateWithDerivativesImpl(points,
                                          derivatives,
                                          derivativeIndices,
                                          degree,
                                          parameters,
